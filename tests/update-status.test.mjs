...
function status(outcome = "succeeded", successAt = "2026-08-18T00:00:00.000Z") {
  return buildPublicUpdateStatus({
    outcome,
    runId: "123",
    runAttempt: 1,
    attemptedAt: "2026-08-18T00:30:00.000Z",
    runUrl: "https://github.com/yagiharuka/meti-funding-watch/actions/runs/123",
    release: releaseIdentity,
    lastSuccessfulImportAt: successAt,
  });
}
test("uses an eight-day stale threshold for the weekly Gbiz refresh", () => {
  assert.equal(PAGE_UPDATE_STALE_AFTER_MS, 8 * 24 * 60 * 60 * 1000);
...
test("status contains no official-series freshness state", () => {
  const value = status();
  assert.equal("official" in value, false);
