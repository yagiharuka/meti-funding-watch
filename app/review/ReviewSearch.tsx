...
const PROGRAM_TARGET_CHANGE_EVENT = "meti-review-program-target-change";

function subscribeToProgramTarget(onStoreChange: () => void) {
  window.addEventListener("popstate", onStoreChange);
  window.addEventListener(PROGRAM_TARGET_CHANGE_EVENT, onStoreChange);
  return () => {
    window.removeEventListener("popstate", onStoreChange);
    window.removeEventListener(PROGRAM_TARGET_CHANGE_EVENT, onStoreChange);
  };
}

function getProgramTargetSnapshot() {
  const candidate = new URLSearchParams(window.location.search).get(REVIEW_PROGRAM_PARAMETER)?.trim() ?? "";
  return candidate.length <= 200 ? candidate : "";
}
...
export default function ReviewSearch() {
  ...
  }, [targetIsVisible, targetProgramId]);
  ...
  return (
    <section className="official-search-section" aria-labelledby="review-search-title">
      ...
      {targetProgram && <p className="direct-program-context"><strong>{targetProgram.name}</strong><span>予算事業ID {targetProgram.projectNumber}／{targetProgram.reviewSheetYear}年度シートに掲載された支出先です。中間・下流など異なる経路の金額は合算しません。</span></p>}
      ...
      {error ? <div className="adoption-error" role="alert"><strong>行政事業レビューを表示できません。</strong><p>{error}</p></div> : (
        <div className={`records-table official-results-table${targetProgramId ? " direct-program-results" : ""}`} id={targetProgramId ? reviewProgramRecipientsAnchorId(targetProgramId) : undefined} role="region" aria-label={targetProgram ? `${targetProgram.name}（${targetProgram.reviewSheetYear}年度シート）の支出先一覧` : "行政事業レビュー検索結果"} tabIndex={0}>
