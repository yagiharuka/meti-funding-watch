...
| M-017 | 出典・鮮度 | 公式資料・公式補足＝経産省関係の全機関・全年の公表資料を網羅 | 高 | トップ説明、公式資料、企業検索の公式補足 | トップで全支出ではないことと公式補足・照合記録の限定性を明示し、公式補足は本省・エネ庁・中企庁・特許庁・NEDO・中小機構・JOGMEC・JETRO・産総研・INPIT・NITE・IPA・RIETIの確認済み公表行だけを表示。地方局等の公式資料キャッシュは企業検索の母集団に含めない | `app/page.tsx`, `app/CombinedCompanyResults.tsx`, `pages-site/company-evidence-ui.ts`, `scripts/build-official-company-index.mjs`, `scripts/build-official-supplement-index.mjs`, `scripts/aist-official-supplement.mjs`, `data/official-supplement-inpit.json`, `data/official-supplement-nite.json`, `data/official-supplement-ipa.json`, `data/official-supplement-rieti.json` | `tests/misreading-catalog.test.mjs`, `tests/company-evidence-ui.test.mjs`, `tests/official-sources.test.mjs`, `tests/official-supplement.test.mjs`, `tests/aist-official-supplement.test.mjs` | MITIGATED |
...
| M-026 | 出典・鮮度 | GitHub上のコミット済み公式補足索引＝現在のビルダが生成する公開用派生データ | 高 | GitHub上のJSON、再利用・監査 | `official-supplement-index.json` を現行ビルダ出力で同期し、CIのclean checkoutで再生成した後にコミット済みファイルとの差分があれば失敗させる。月次公式更新でも同派生索引を明示的なcommit対象として維持する | `scripts/build-official-supplement-index.mjs`, `.github/workflows/ci.yml`, `.github/workflows/refresh-official-data.yml` | CI `Verify committed official supplement index` | MITIGATED |
