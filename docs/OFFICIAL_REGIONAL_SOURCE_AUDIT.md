...
## 関東局PDFの公式目次inventoryと最小安全wave

2026-06-01 09:34:42のWARP captureに保存された関東局の公式目次をFull GETした。交付決定目次は23,677 bytes・SHA-256 `09ea02e5a55136947de3552b193d7465e6e04e6ad1be2e8421288e2a06ce88bb`、契約結果目次は13,257 bytes・SHA-256 `df20f1093577671eec6aed307e53464b36205a2569047c77652146e8aefd65ff` に固定した。目次上のPDF hrefを年度・区分ごとに数えたinventoryは次のとおりである。

| 区分 | 公式目次で確認した範囲 | PDF href | 本番取込 | 残件 |
|---|---|---:|---:|---:|
| 補助金等の交付決定 | 令和3～7年度 | 41 | 令和7年度8 | 令和3～6年度33 |
| 契約締結状況 | 令和4～7年度、競争/随意×物品役務/委託 | 16 | 0 | 16 |
| 合計 | 公式目次hrefのみ（推測URLなし） | 57 | 8 | 49 |

令和7年度の交付決定欄には上期5 href、下期3 hrefがあり、8/8件を取り込んだ。下期のエネルギー需給勘定（補助金）・特許特別会計（補助金）は目次にhrefがないため、ファイル名を推測して0件扱いしたり収録したりしない。
