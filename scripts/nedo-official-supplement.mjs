...
export async function refreshNedoOfficialSupplement({ fetchImpl = fetch, outputPath = OUTPUT_PATH } = {}) {
  const seeds = JSON.parse(await readFile(outputPath, "utf8"));
  if (seeds.schemaVersion !== 1 || !Array.isArray(seeds.sources)) throw new Error("公式補足シードの形式が不正です");
  const source = seeds.sources.find((item) => item.id === "nedo");
  if (!source || !Array.isArray(source.records)) throw new Error("NEDO公式補足シードがありません");
  ...
  if (parsed.length < source.records.length || parsed.length < 5) {
    throw new Error(`NEDO GX解析件数が既存収録を下回りました: ${parsed.length}/${source.records.length}\n${failures.slice(0, 10).join("\n")}`);
  }

  source.records = mergeNedoRecords(source.records, parsed);
  source.coverageNote = `NEDOのDTSU・GX採択事業者検索サイトから、GX分野のディープテック・スタートアップ支援について企業名・研究開発テーマ・フェーズ・事業年度・交付決定額を定型HTMLで継続取得。今回 ${links.length}ページを確認し、GX ${parsed.length}件を解析。過去に確認済みの行は一覧掲載終了後も保持する。NEDO全事業・全契約を網羅するものではない。`;
  seeds.updatedAt = new Date().toISOString();
  await writeFile(outputPath, `${JSON.stringify(seeds)}\n`);
  return { source, listingCount: links.length, parsedCount: parsed.length, failureCount: failures.length };
}

async function main() {
  const result = await refreshNedoOfficialSupplement();
  console.log(`NEDO official supplement: ${result.source.records.length} retained / ${result.parsedCount}/${result.listingCount} GX parsed`);
}
