...
export function currentInpitFiscalPage(now = new Date()) {
  ...
  return { fiscalYear, url: `https://www.inpit.go.jp/kobo/contract_info/r${era}/index.html` };
}

function normalizeSectionTitle(value = "") {
  return htmlToText(value).replace(/[：]/g, ":").replace(/\s+/g, "").replace(/^###?/u, "");
}
...
function positionedItems(items) {
  return items
    ...
    .map((item) => ({ text: normalizeText(item.str), x: Number(item.transform[4]), y: Number(item.transform[5]) }))
...
function findHeader(items, pageNumber) {
  ...
  const ordered = ["program", "date", "organization", "amount", "remarks"]
  ...
  return { ordered, headerY: Math.min(...ordered.map((item) => item.y)) };
}
...
function columnBounds(header) {
  ...
  for (let index = 0; index < header.ordered.length; index += 1) {
    bounds[header.ordered[index].key] = {
...
function parseParty(value, context) {
  ...
  const organization = organizationRaw.replace(/\(株\)|㈱/gu, "株式会社").replace(/\(有\)|㈲/gu, "有限会社").trim();
